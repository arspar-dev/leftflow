...
export function ServicesSection({ dict }: Props) {
  return (
    <section id="services" className="py-20 lg:py-32 bg-charcoal-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn>
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 text-primary-400 text-sm font-medium mb-3">
                ...
                <div className={`flex flex-col ${isReversed ? "lg:flex-row-reverse" : "lg:flex-row"} gap-8 lg:gap-12 items-center`}>
                  <div className="flex-1 space-y-4">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-xl ${iconColors[i]} flex items-center justify-center`}>
                        {icons[i]}
                      </div>
                      <span className="text-sm font-mono text-charcoal-400 tracking-wider">{item.number}</span>
                    </div>
                    <h3 className="text-2xl font-bold text-white">{item.title}</h3>
                    <p className="text-charcoal-400 leading-relaxed text-lg">{item.description}</p>
                  </div>
                  <div className="flex-1 w-full">
                    <div className="relative rounded-2xl overflow-hidden border border-charcoal-700/50 shadow-2xl aspect-video bg-charcoal-900">
                      <video
